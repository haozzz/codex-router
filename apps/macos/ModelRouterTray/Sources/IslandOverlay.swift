...
private struct IslandOverlayView: View {
    ...
    HStack(spacing: 7) {
      ...
      if !store.activeRequests.isEmpty {
        Label("\(activeSessions.count)", systemImage: "bubble.left.and.bubble.right.fill")
          ...
          .fixedSize()
          .help("\(activeSessions.count) 个会话运行中")
      }
      if store.activeRequests.isEmpty {
  ...
  private var usagePeekContent: some View {
    VStack(spacing: 9) {
      HStack(spacing: 9) {
        ...
        Spacer()
        HStack(spacing: 12) {
          IslandHeaderMetric(value: todayTokenValue, label: "今日 Token")
      ...
      HStack(spacing: 9) {
        ...
        VStack(alignment: .leading, spacing: 1) {
          Text(store.activityState.label)
            .font(.system(size: 12, weight: .semibold, design: .rounded))
            .foregroundStyle(store.activityState.tint)
          Text("\(activeSessions.count) 个会话运行中")
  ...
  private var usageExpandedContent: some View {
    VStack(spacing: 13) {
      HStack(spacing: 10) {
        ...
        Spacer()
        Button("收起") { display.setState(.peek) }
        .buttonStyle(.plain)
        .font(.system(size: 9, weight: .medium, design: .rounded))
        .foregroundStyle(routerMuted)
      }
      ...
      HStack {
        Text(store.hasConcurrentActivity ? "当前活跃" : "当前提供商")
          .font(.system(size: 8, weight: .semibold, design: .monospaced))
          .tracking(0.8)
          .foregroundStyle(routerMuted)
        Spacer()
      ...
      if store.hasConcurrentActivity {
        ...
        HStack {
          ...
          Spacer()
          Text(store.activityState == .generating ? "实时" : "上次使用")
            .font(.system(size: 9, weight: .medium, design: .rounded))
            .foregroundStyle(store.activityState.tint)
        }
  ...
  private var sessionExpandedContent: some View {
    VStack(spacing: 12) {
      HStack(spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 2) {
          Text(selectedSession?.name ?? "运行中的会话")
            .font(.system(size: 15, weight: .semibold, design: .rounded))
            .lineLimit(1)
          Text(selectedSession == nil
            ? "\(activeSessions.count) 个会话运行中"
            : "\(selectedSession?.agents.count ?? 0) 个已分配代理")
            .font(.system(size: 9, weight: .medium, design: .rounded))
            .foregroundStyle(store.activityState.tint)
        }
        Spacer()
  ...
  private var activeSessions: [IslandActivitySession] {
    ...
    return grouped.map { id, requests in
      let fallback = requests.first.map(store.sessionName(for:)) ?? "活动会话"
  ...
  private var sourceLabel: String {
    ...
    if provider == "openai" { return "ChatGPT · 原生" }
    if provider == "grok-oauth" { return "XAI · OAuth 会话" }
    if provider == "grok-api" { return "XAI · 按量 API" }
  ...
  private var accountUsageLabel: String {
    if store.selectedUsageUsesChatGPT {
      return localizedLimitLabel(store.accountUsage?.primary?.durationLabel ?? "ChatGPT limit")
    }
    if let metric = store.selectedAccountMetric {
      if metric.kind == "quota" { return localizedLimitLabel(standardizedLimitLabel(metric.label)) }
      if metric.kind == "balance" { return "余额" }
      return metric.label
    }
    return "用量限额"
  }
  ...
  private var accountHeaderLabel: String {
    if weeklyRemainingPercent != nil { return "每周剩余" }
    if quotaUsedPercent != nil {
      let window = accountUsageLabel
        .replacingOccurrences(of: " limit", with: "", options: [.caseInsensitive])
        .replacingOccurrences(of: "限额", with: "")
      return window.isEmpty ? "已用" : "\(window)已用"
    }
    return accountUsageLabel
  }
  ...
  private var accountTileValue: String {
    if let quotaUsedPercent { return "\(Int(quotaUsedPercent.rounded()))% 已用" }
  ...
  private var accountTileDetail: String {
    ...
    return quotaUsedPercent == nil ? "提供商未报告" : "未报告重置时间"
  }

  private var tokenSourceDetail: String {
    store.selectedUsageUsesChatGPT ? "ChatGPT 账户用量" : "由本 Router 测量"
  }
...
private struct IslandUsageLineChart: View {
  ...
  var body: some View {
    ...
    .accessibilityLabel("每日 Token 用量折线图")
    .accessibilityValue("\(points.count) 天共 \(formattedTotalTokens) Token")
  }
...
private struct ProviderIcon: View {
  ...
  private var assetName: String? {
    ...
    if providerID == "volcengine-plan" { return "volcengine-plan" }
    return nil
  }

  private var assetExtension: String {
    ["github-copilot", "chutes", "volcengine-plan"].contains(providerID) ? "svg" : "png"
  }
  ...
  private var providerName: String {
    ...
    if providerID == "chutes" { return "Chutes" }
    if providerID == "volcengine-plan" { return "火山方舟" }
    return "模型提供商"
  }
}
...
private struct IslandSessionList: View {
  ...
  private func shortModelSummary(_ session: IslandActivitySession) -> String {
    ...
    guard let first = models.first else { return "活跃" }
...
private struct IslandAgentList: View {
  ...
  private func agentLabel(_ request: RouterActiveRequest) -> String {
    ...
    return request.isSubagent == true ? "子智能体" : "主代理"
  }
    ...
    VStack(spacing: compact ? 5 : 6) {
      ForEach(Array(store.activeRequests.prefix(limit))) { request in
        HStack(spacing: 8) {
          ...
          .frame(maxWidth: .infinity)
          TimelineView(.periodic(from: .now, by: 1)) { context in
            Text("思考中 · \(elapsedLabel(for: request, now: context.date))")
      ...
      if store.activeRequests.count > limit {
        Text("+\(store.activeRequests.count - limit) 个更多")
          .font(.system(size: 9, weight: .medium, design: .rounded))
          .foregroundStyle(routerMuted)
          .frame(maxWidth: .infinity, alignment: .leading)
      }
